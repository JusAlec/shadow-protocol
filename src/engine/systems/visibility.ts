...
export function hasLineOfSight(
  ...
  while (true) {
    ...
    if (!(x0 === from.x && y0 === from.y) && !(x0 === to.x && y0 === to.y)) {
      ...
      if (tile && tile.smoke && tile.smoke > 0) return false;
    }
...
// Check if smoke specifically blocks the line of sight between two points
export function isSmokeBlocking(from: Position, to: Position, grid: TileData[][]): boolean {
  let x0 = from.x, y0 = from.y;
  const x1 = to.x, y1 = to.y;
  const dx = Math.abs(x1 - x0);
  const dy = Math.abs(y1 - y0);
  ...
  while (true) {
    if (!(x0 === from.x && y0 === from.y) && !(x0 === to.x && y0 === to.y)) {
      const tile = grid[y0]?.[x0];
      if (tile && tile.smoke && tile.smoke > 0) return true;
    }
    if (x0 === x1 && y0 === y1) break;
    const e2 = 2 * err;
    if (e2 > -dy) { err -= dy; x0 += sx; }
